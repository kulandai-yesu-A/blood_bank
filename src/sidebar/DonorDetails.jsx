...
const DonorTable = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        //hidden:Yup.string().required('required'),
    ...
    const updateDonor = async (data) => {
        try {
            const formattedData = formatDonorData(data);
            const response = await axios.put(`${API_URL}${editingDonor.id}/`, formattedData);
            setDonors(donors.map((donor) => (donor.id === editingDonor.id ? response.data : donor)));
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row) => (
                <>
                    <button onClick={() => handleEdit(row)} className="btn btn-warning btn-sm">Edit</button>
                    <button onClick={() => deleteDonor(row.id)} className="btn btn-danger btn-sm">Delete</button>
                </>
            ),
        },
    ];
                ...
                <div className="col-sm-9">
                    ...
                    {showForm && (
                        <form onSubmit={handleSubmit(onSubmit)} className="mb-4">
                            ...
                            <div className="mb-3">
                                {/* <label> Status</label> */}
                                <input type="hidden" {...register('status')} className="form-control" value={'Pending'}/>
                            </div>
